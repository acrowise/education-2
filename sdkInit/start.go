...
func CreateChannel(sdk *fabsdk.FabricSDK, info *InitInfo) error {
	...
	if err != nil {
		return fmt.Errorf("應用通道建立失敗!: %v", err)
	}

	fmt.Println("應用通道建立成功!")
...
func InstallAndInstantiateCC(sdk *fabsdk.FabricSDK, info *InitInfo) (*channel.Client, error) {
	fmt.Println("開始安裝智能合約......")
	// creates new go lang chaincode package
	ccPkg, err := gopackager.NewCCPackage(info.ChaincodePath, info.ChaincodeGoPath)
	if err != nil {
		return nil, fmt.Errorf("建立智能合約失敗!: %v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("安裝智能合約失敗!: %v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("實例化智能合約失敗!: %v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("建立應用通道客戶端失敗!: %v", err)
	}

	fmt.Println("應用通道客戶端建立成功,可以利用此客戶端調用智能合約進行查詢或交易")
